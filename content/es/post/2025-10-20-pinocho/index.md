...
1. Corte por Mediana: El test ignora los valores y mira solo si cada dato está por encima (`\(\text{+}\)`) o por debajo (`\(\text{-}\)`) de la mediana (el valor central)

2.Rachas: Cuenta cuántas veces hay un cambio de signo (ej. `\(\text{+ + - - - +}\)` tiene tres rachas: `\(\text{++}\)`, `\(\text{---}\)`, `\(\text{+}\)`).

- Pocas Rachas (Agrupamiento): Indica una tendencia a que los valores se agrupen (ej. `\(\text{+ + + - - -}\)`), lo que sugiere pereza o un patrón lento.

- Muchas Rachas (Alternancia): Indica que los valores suben y bajan demasiado rápido (ej. `\(\text{+ - + - + -}\)`), lo que sugiere que la persona está forzando la aleatoriedad.
