...
## Quinta prueba: Análisis de Distribución (La forma de la mentira)

Hasta ahora hemos mirado números aislados. Ahora miremos la "silueta" completa de los datos.

En estadística, la forma de la curva nos cuenta la actitud del autor:
...
```{r g_curtosis}
# --- CÓDIGO NUEVO DEL GRÁFICO DE DENSIDAD ---
# (Pega aquí el código que te di en la respuesta anterior para generar g_curtosis)
# Si ya lo tienes en analisis.R, solo llámalo:
g_curtosis
```
...
{{% callout warning %}}
Diagnóstico: Cada participante dejó una "huella de forma" distinta.
{{% /callout %}}

En la figura: 

La Realidad (Gris): Es una colina suave y centrada en 136. Acepta valores bajos y altos con naturalidad.

La Prudente (Verde): ¡Es una aguja!

Tiene lo que llamamos Leptocurtosis extrema.
Su curva es altísima y estrecha. Al intentar "no fallar", se quedó paralizada en el centro. Es la gráfica del miedo al riesgo.
La Confiada (Azul): Es un camello deforme.

Observen cómo su curva se derrite hacia la izquierda (valores bajos, 100-110).
Esto es Asimetría Negativa. Su conocimiento médico la traicionó: como sabe que la anemia es común, inventó demasiados valores bajos, deformando la curva lejos de la normalidad.
La Entusiasta (Rojo): La imitadora.

Es la única que logró una forma similar a la campana real, aunque un poco más estrecha. Su estrategia de "fuerza bruta" (escribir rápido sin pensar) fue la que mejor replicó la naturaleza.
