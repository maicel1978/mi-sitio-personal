...
> descrito con suficiente detalle en el protocolo" o, preferiblemente, como un Plan de Análisis Estadístico (SAP) independiente y consistente, respondiendo a la pregunta crítica de "¿Cómo demostrar que los resultados no fueron 'inventados' tras ver los datos?.
...
-   **Cálculo Insuficiente del Tamaño Muestral** (`\(\text{N}\)`): El cálculo incorrecto o injustificado del `\(\text{N}\)` es el error estadístico central, comprometiendo gravemente la potencia estadística del estudio.
...
{{% callout note %}} Los reguladores rechazan protocolos con objetivos ambiguos o mal definidos, y aquellos que carecen de una justificación estadística robusta. El cálculo muestral inadecuado compromete la potencia estadística, que es la capacidad del estudio para obtener una respuesta fiable. {{% /callout %}}

#### La Absolución y Redención:

La redención requiere someterse a la humildad mediante la revisión crítica y externa, idealmente de un bioestadístico independiente, antes de la presentación. Esto incluye:

-   Desarrollar el PAE/SAP de forma independiente y consistente con el protocolo.

-   Justificar y documentar los supuestos del cálculo del tamaño muestral.

**Virtud opuesta:** Humildad y Rigor. *"El protocolo que se cree infalible invariablemente termina marcado en rojo por un revisor misericordioso."* —Así, el investigador se redime, fortaleciendo el diseño y facilitando la aprobación regulatoria.
...
> La ICH E6(R3), sección 3.16.2(f), exige trazabilidad: todo cambio en el Plan de Análisis Estadístico (PAS) debe estar documentado, fechado y justificado. El protocolo perezoso no falla por ignorancia, sino por omisión.
...
-   Inconsistencia entre Protocolo y PAS: Existe una falta de armonización entre lo descrito en el protocolo metodológico y el plan de análisis estadístico detallado. Esto se produce por una falta de planificación multidisciplinaria y control de calidad documental.
...
-   Armoniza: Asegura la coherencia entre el Protocolo, el PAS, y el Formulario de Recolección de Datos (CRF).
...
Hablar de las pruebas de significación y de la necesidad de poner medidas de tamaño del efecto como odd- ratio

Tambien sobre las formulas del tamaño de muestra, y porque es un ritual estaditicos

De que muchos informes presentan varias medidas de tendencia central que se repiten varias veces
