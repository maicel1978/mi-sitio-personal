...
# 200 pacientes
n <- 200
grupo <- rep(c("Control", "Experimental"), each = n/2)
baseline <- rnorm(n, 50, 10)

# Respuesta al tiempo intermedio
week_interim <- baseline + ifelse(grupo=="Experimental", rnorm(n,4,3), rnorm(n,1.5,3))

# No respondedores del control cambian al experimental
non_resp <- grupo=="Control" & (week_interim - baseline < 3)
grupo_post <- grupo
grupo_post[non_resp] <- "Switch"

# Resultado final
week_final <- week_interim + ifelse(grupo_post=="Experimental", rnorm(n,2,3),
                                    ifelse(grupo_post=="Switch", rnorm(n,4.5,3), rnorm(n,0.5,3)))

data <- data.frame(grupo, grupo_post, baseline, week_interim, week_final)

# Promedios por grupo
summary_data <- data %>%
  group_by(grupo_post) %>%
  summarise(Baseline=mean(baseline),
            Intermedio=mean(week_interim),
            Final=mean(week_final)) %>%
  tidyr::pivot_longer(-grupo_post, names_to="Tiempo", values_to="Media")

# Gráfico
ggplot(summary_data, aes(x=Tiempo, y=Media, group=grupo_post, color=grupo_post)) +
  geom_line(size=1.2) + geom_point(size=3) +
  theme_minimal() +
  labs(title="Rescate ético vs inferencia causal",
       y="Outcome medio",
       color="Grupo final")
```
