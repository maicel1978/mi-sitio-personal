...
tabla_media %>% 
  ...
  footnote(
    ...
    general = "Estimation Error = Estimated Parameter (Arithmetic Mean (Hb g/l)) - Population Parameter (Hb: 136 g/l)  ", 
    general_title = "note:",
    footnote_as_chunk = T 
  )
```
...
tabla_ultimo_digito %>% 
  kable(
    ...
    align = "c",
    col.names = c("Digit", "The Confident One", "The Enthusiastic One", "The Cautious One")
  ) %>%
...
tabla_rachas_display %>% 
  select(- "Evaluación" ) %>% 
  kable(col.names = c("Participant", "Z-Score"))
```
